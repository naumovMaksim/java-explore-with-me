...
import ru.practicum.main_service.event.model.RequestStats;
...
public interface RequestRepository extends JpaRepository<Request, Long> {
    ...
    @Query("SELECT new ru.practicum.main_service.event.model.RequestStats(r.event.id, COUNT(r.id)) " +
            "FROM Request AS r " +
            "WHERE r.event.id IN ?1 AND r.status = 'CONFIRMED' " +
            "GROUP BY r.event.id")
    List<RequestStats> getConfirmedRequests(List<Long> eventIds);
}
